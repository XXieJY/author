...
class Tongren extends Controller{
    ...
    public function solid(){
        $solid =Db::name('BookPromote')->where(['promote_id'=>13])->field('book_id,book_name,upload_img')->limit(5)->select();
        return $solid;
    }
    ...
    public function god_touch(){
        $touch =Db::view('Book','book_id,book_name,book_brief,words')
                ...
                ->where(['Book.type_id'=>16,'BookPromote.promote_id'=>3])
                ->order('xu asc')
                ->limit(8)
                ->select();
        return $touch;
    ...
    public function lianzai(){
        ...
        $lianzai =Db::name('Book')->where($where)
                 ->field('book_id,book_name,author_name,upload_img,words,level,book_brief')
    ...
    public function newBook(){
        ...
        $newBook =Db::name('Book')->where($where)
            ->field('book_id,book_name,author_name,upload_img,words,level,book_brief')
            ->order('update_time desc')->limit(12)->select();
        return $newBook;
