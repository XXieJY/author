    ...
    public function user($id){
        ...
        if(request()->isPost()){
           ...
           $result= Db::name('Writer')->where(['pen_name'=>$data['uname']])->find();
            ...
            $con['uid']=1;
            $con['pen_name']=trim($data['uname']);
            $con['user_name']=trim($data['name']);
            $con['card']      =trim($data['idCard']);
            $con['qq']    =trim($data['qq']);
            $con['sign'] =trim($data['signature']);
            $con['update_time'] =date('Y-m-d H:i:s');
            //数据入库
          $ok=  Db::name('Writer')->where(['author_id'=>$data['id']])->update($con);
          if($ok){
              $writer=Db::name('Writer')->where(['author_id'=>$data['id']])->find();

              Db::startTrans();//开启事务
              try{
                  $userid=  \think\Loader::controller('gongju/User')->add($writer['phone'],$writer['email'],$writer['pen_name']);
                  $ok= model('Writer')->getUser($data['id'],$userid);
                  if($userid && $ok){
                      Db::commit();//提交事务
